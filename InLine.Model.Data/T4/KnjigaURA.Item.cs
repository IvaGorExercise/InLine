//------------------------------------------------------------------------------
// <auto-generated>
//    This code was generated from a template.
//
//    Manual changes to this file may cause unexpected behavior in your application.
//    Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace InLine.Model.Data.Item
{
    using System;
    using System.Collections.Generic;
    
    public partial class KnjigaURAItem 
    {
    
    	public KnjigaURAItem() {}
    	public KnjigaURAItem(long? id_KnjigaURA)
    	{
    		ID_KnjigaURA = id_KnjigaURA;
    	}
    
        public long? ID
    	{
    		get { return this.ID_KnjigaURA; }
    		set { this.ID_KnjigaURA = value; }
    	}
        public long? ID_KnjigaURA { get; set; }
        public long? ID_Zapis { get; set; }
        public long? ID_PoslovnaGodina { get; set; }
        public string Oznaka { get; set; }
        public string Naziv { get; set; }
        public string NazivKr { get; set; }
        public string Kratica { get; set; }
        public long? ID_RacunUlaz { get; set; }
    
    	public KnjigaURAItem(Model.Data.KnjigaURA item)
    	{
    	    this.ID_KnjigaURA = item.ID_KnjigaURA;
        this.ID_Zapis = item.ID_Zapis;
        this.ID_PoslovnaGodina = item.ID_PoslovnaGodina;
        this.Oznaka = item.Oznaka;
        this.Naziv = item.Naziv;
        this.NazivKr = item.NazivKr;
        this.Kratica = item.Kratica;
        this.ID_RacunUlaz = item.ID_RacunUlaz;
    	}
    
    	public KnjigaURAItem(dynamic item)
    	{
    	    this.ID_KnjigaURA = item.ID_KnjigaURA;
        this.ID_Zapis = item.ID_Zapis;
        this.ID_PoslovnaGodina = item.ID_PoslovnaGodina;
        this.Oznaka = item.Oznaka;
        this.Naziv = item.Naziv;
        this.NazivKr = item.NazivKr;
        this.Kratica = item.Kratica;
        this.ID_RacunUlaz = item.ID_RacunUlaz;
    	}
    }
}
