...
namespace InLine.UI.Web.Controllers
{
    ...
    public partial class PrimitakGrPrimitakGrController : Controller
    {
    	
    	#region Fields
    
    	 Service.Data.IPrimitakGrPrimitakGr _service;
    	...
    	public ActionResult Insert(InLine.Model.Data.Item.PrimitakGrPrimitakGrItem item)
    	{
    		
    		if (item.ID_PrimitakGrPrimitakGr == -1)
    		{
    	...
    	public ActionResult Delete(InLine.Model.Data.Item.PrimitakGrPrimitakGrItem item)
    	{
        ...
        public InLine.Model.Data.PrimitakGrPrimitakGr FindID(long id)
        {
    		InLine.Model.Data.PrimitakGrPrimitakGr rezultat;
            rezultat = _service.FindID(id);
            return rezultat;  
        }
    	
    	
        //public IEnumerable<InLine.Model.Data.PrimitakGrPrimitakGr> GetGridData(string filter, string sort, long? start, long? count, bool customsearch = false)
        //{
        //    IEnumerable<InLine.Model.Data.PrimitakGrPrimitakGr> rezultat;
         //   rezultat = _service.GetGridData(filter, sort, start, count, customsearch);
         //   return rezultat;  
        //}
    
    	public IEnumerable<InLine.Model.Data.PrimitakGrPrimitakGr> GetGridData(string filter, string key, string value, string sort, long? start, long? count, bool customsearch = false)
        {
            IEnumerable<InLine.Model.Data.PrimitakGrPrimitakGr> rezultat;
    	...
    	public ActionResult GetGrid(JqGridRequest request, string key, string value, string viewModel, string field, long? id_master)
        {
            ...
            {
                fieldMain = "ID_PrimitakGrPrimitakGr";
            }
    		...
    		public static string GetFilter(string searchingName, JqGridSearchOperators searchingOperator, string searchingValue)
    		{
    			...
    			switch (searchingOperator)
    			{
    				...
    				if (searchingName == "ID_PrimitakGrPrimitakGr")
    				{
    					return String.Format("{0} {1} {2}", searchingName, searchingOperatorString, searchingValue);
    				}
    				if (searchingName == "ID_PrimitakGr")
    				{
    					return String.Format("{0} {1} {2}", searchingName, searchingOperatorString, searchingValue);
    				}
    				if (searchingName == "ID_PrimitakGrNad")
    				{
    					return String.Format("{0} {1} {2}", searchingName, searchingOperatorString, searchingValue);
    				}
    	...
    	#region Akcije
    	    ...
    	    protected override void OnActionExecuting(ActionExecutingContext filterContext)
            {
                ...
                if (korisnik_data != null)
                {
                   ...
                   _service = new Service.Data.PrimitakGrPrimitakGr(korisnik_data);
                }
    			else
                {
