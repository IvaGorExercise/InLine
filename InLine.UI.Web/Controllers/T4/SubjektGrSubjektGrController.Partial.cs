...
namespace InLine.UI.Web.Controllers
{
    ...
    public partial class SubjektGrSubjektGrController : Controller
    {
    	
    	#region Fields
    
    	 Service.Data.ISubjektGrSubjektGr _service;
    	...
    	public ActionResult Insert(InLine.Model.Data.Item.SubjektGrSubjektGrItem item)
    	{
    		
    		if (item.ID_SubjektGrSubjektGr == -1)
    		{
    	...
    	public ActionResult Delete(InLine.Model.Data.Item.SubjektGrSubjektGrItem item)
    	{
        ...
        public InLine.Model.Data.SubjektGrSubjektGr FindID(long id)
        {
    		InLine.Model.Data.SubjektGrSubjektGr rezultat;
            rezultat = _service.FindID(id);
            return rezultat;  
        }
    	
    	
        //public IEnumerable<InLine.Model.Data.SubjektGrSubjektGr> GetGridData(string filter, string sort, long? start, long? count, bool customsearch = false)
        //{
        //    IEnumerable<InLine.Model.Data.SubjektGrSubjektGr> rezultat;
         //   rezultat = _service.GetGridData(filter, sort, start, count, customsearch);
         //   return rezultat;  
        //}
    
    	public IEnumerable<InLine.Model.Data.SubjektGrSubjektGr> GetGridData(string filter, string key, string value, string sort, long? start, long? count, bool customsearch = false)
        {
            IEnumerable<InLine.Model.Data.SubjektGrSubjektGr> rezultat;
    	...
    	public ActionResult GetGrid(JqGridRequest request, string key, string value, string viewModel, string field, long? id_master)
        {
            ...
            {
                fieldMain = "ID_SubjektGrSubjektGr";
            }
    		...
    		public static string GetFilter(string searchingName, JqGridSearchOperators searchingOperator, string searchingValue)
    		{
    			...
    			switch (searchingOperator)
    			{
    				...
    				if (searchingName == "ID_SubjektGrSubjektGr")
    				{
    					return String.Format("{0} {1} {2}", searchingName, searchingOperatorString, searchingValue);
    				}
    				if (searchingName == "ID_SubjektGr")
    				{
    					return String.Format("{0} {1} {2}", searchingName, searchingOperatorString, searchingValue);
    				}
    				if (searchingName == "ID_SubjektGrNad")
    				{
    					return String.Format("{0} {1} {2}", searchingName, searchingOperatorString, searchingValue);
    				}
    	...
    	#region Akcije
    	    ...
    	    protected override void OnActionExecuting(ActionExecutingContext filterContext)
            {
                ...
                if (korisnik_data != null)
                {
                   ...
                   _service = new Service.Data.SubjektGrSubjektGr(korisnik_data);
                }
    			else
                {
