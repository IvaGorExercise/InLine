...
namespace InLine.UI.Web.Controllers
{
    ...
    public partial class LokacijaLokacijaController : Controller
    {
    	
    	#region Fields
    
    	 Service.Data.ILokacijaLokacija _service;
    	...
    	public ActionResult Insert(InLine.Model.Data.Item.LokacijaLokacijaItem item)
    	{
    		
    		if (item.ID_LokacijaLokacija == -1)
    		{
    	...
    	public ActionResult Delete(InLine.Model.Data.Item.LokacijaLokacijaItem item)
    	{
        ...
        public InLine.Model.Data.LokacijaLokacija FindID(long id)
        {
    		InLine.Model.Data.LokacijaLokacija rezultat;
            rezultat = _service.FindID(id);
            return rezultat;  
        }
    	
    	
        //public IEnumerable<InLine.Model.Data.LokacijaLokacija> GetGridData(string filter, string sort, long? start, long? count, bool customsearch = false)
        //{
        //    IEnumerable<InLine.Model.Data.LokacijaLokacija> rezultat;
         //   rezultat = _service.GetGridData(filter, sort, start, count, customsearch);
         //   return rezultat;  
        //}
    
    	public IEnumerable<InLine.Model.Data.LokacijaLokacija> GetGridData(string filter, string key, string value, string sort, long? start, long? count, bool customsearch = false)
        {
            IEnumerable<InLine.Model.Data.LokacijaLokacija> rezultat;
    	...
    	public ActionResult GetGrid(JqGridRequest request, string key, string value, string viewModel, string field, long? id_master)
        {
    		...
    		public static string GetFilter(string searchingName, JqGridSearchOperators searchingOperator, string searchingValue)
    		{
    			...
    			switch (searchingOperator)
    			{
    				...
    				if (searchingName == "ID_LokacijaLokacija")
    				{
    					return String.Format("{0} {1} {2}", searchingName, searchingOperatorString, searchingValue);
    				}
    				if (searchingName == "ID_LokacijaLokacijaStruktura")
    				{
    					return String.Format("{0} {1} {2}", searchingName, searchingOperatorString, searchingValue);
    				}
    				if (searchingName == "ID_Lokacija")
    				{
    					return String.Format("{0} {1} {2}", searchingName, searchingOperatorString, searchingValue);
    				}
    				if (searchingName == "ID_LokacijaNad")
    				{
    					return String.Format("{0} {1} {2}", searchingName, searchingOperatorString, searchingValue);
    				}
    				
    		
    					if (searchingName == "Nivo")
    				{
    	...
    	#region Akcije
    	    ...
    	    protected override void OnActionExecuting(ActionExecutingContext filterContext)
            {
                ...
                if (korisnik_data != null)
                {
                   ...
                   _service = new Service.Data.LokacijaLokacija(korisnik_data);
                }
    			else
                {
