...
namespace InLine.UI.Web.Controllers
{
    ...
    public partial class DatotekaPoljeController : Controller
    {
    	
    	#region Fields
    
    	 Service.Data.IDatotekaPolje _service;
    	...
    	public ActionResult Insert(InLine.Model.Data.Item.DatotekaPoljeItem item)
    	{
    		
    		if (item.ID_DatotekaPolje == -1)
    		{
    	...
    	public ActionResult Delete(InLine.Model.Data.Item.DatotekaPoljeItem item)
    	{
        ...
        public InLine.Model.Data.DatotekaPolje FindID(long id)
        {
    		InLine.Model.Data.DatotekaPolje rezultat;
            rezultat = _service.FindID(id);
            return rezultat;  
        }
    	
    	
        //public IEnumerable<InLine.Model.Data.DatotekaPolje> GetGridData(string filter, string sort, long? start, long? count, bool customsearch = false)
        //{
        //    IEnumerable<InLine.Model.Data.DatotekaPolje> rezultat;
         //   rezultat = _service.GetGridData(filter, sort, start, count, customsearch);
         //   return rezultat;  
        //}
    
    	public IEnumerable<InLine.Model.Data.DatotekaPolje> GetGridData(string filter, string key, string value, string sort, long? start, long? count, bool customsearch = false)
        {
            IEnumerable<InLine.Model.Data.DatotekaPolje> rezultat;
    	...
    	public ActionResult GetGrid(JqGridRequest request, string key, string value, string viewModel, string field, long? id_master)
        {
    		...
    		public static string GetFilter(string searchingName, JqGridSearchOperators searchingOperator, string searchingValue)
    		{
    			...
    			switch (searchingOperator)
    			{
    				...
    				if (searchingName == "ID_DatotekaPolje")
    				{
    					return String.Format("{0} {1} {2}", searchingName, searchingOperatorString, searchingValue);
    				}
    				if (searchingName == "ID_Datoteka")
    				{
    					return String.Format("{0} {1} {2}", searchingName, searchingOperatorString, searchingValue);
    				}
    				if (searchingName == "ID_DatotekaVrPolje")
    				{
    					return String.Format("{0} {1} {2}", searchingName, searchingOperatorString, searchingValue);
    				}
    				
    		
    					if (searchingName == "ValueInt")
    				{
    				...
    				if (searchingName == "ValueBit")
    				{
    					return String.Format("{0} {1} {2}", searchingName, searchingOperatorString, searchingValue);
    				}
    				
    
    				if (searchingName == "ValueString")
    			{
    	...
    	#region Akcije
    	    ...
    	    protected override void OnActionExecuting(ActionExecutingContext filterContext)
            {
                ...
                if (korisnik_data != null)
                {
                   ...
                   _service = new Service.Data.DatotekaPolje(korisnik_data);
                }
    			else
                {
