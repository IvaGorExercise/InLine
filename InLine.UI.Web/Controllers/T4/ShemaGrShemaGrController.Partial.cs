...
namespace InLine.UI.Web.Controllers
{
    ...
    public partial class ShemaGrShemaGrController : Controller
    {
    	
    	#region Fields
    
    	 Service.Data.IShemaGrShemaGr _service;
    	...
    	public ActionResult Insert(InLine.Model.Data.Item.ShemaGrShemaGrItem item)
    	{
    		
    		if (item.ID_ShemaGrShemaGr == -1)
    		{
    	...
    	public ActionResult Delete(InLine.Model.Data.Item.ShemaGrShemaGrItem item)
    	{
        ...
        public InLine.Model.Data.ShemaGrShemaGr FindID(long id)
        {
    		InLine.Model.Data.ShemaGrShemaGr rezultat;
            rezultat = _service.FindID(id);
            return rezultat;  
        }
    	
    	
        //public IEnumerable<InLine.Model.Data.ShemaGrShemaGr> GetGridData(string filter, string sort, long? start, long? count, bool customsearch = false)
        //{
        //    IEnumerable<InLine.Model.Data.ShemaGrShemaGr> rezultat;
         //   rezultat = _service.GetGridData(filter, sort, start, count, customsearch);
         //   return rezultat;  
        //}
    
    	public IEnumerable<InLine.Model.Data.ShemaGrShemaGr> GetGridData(string filter, string key, string value, string sort, long? start, long? count, bool customsearch = false)
        {
            IEnumerable<InLine.Model.Data.ShemaGrShemaGr> rezultat;
    	...
    	public ActionResult GetGrid(JqGridRequest request, string key, string value, string viewModel, string field, long? id_master)
        {
    		...
    		public static string GetFilter(string searchingName, JqGridSearchOperators searchingOperator, string searchingValue)
    		{
    			...
    			switch (searchingOperator)
    			{
    				...
    				if (searchingName == "ID_ShemaGrShemaGr")
    				{
    					return String.Format("{0} {1} {2}", searchingName, searchingOperatorString, searchingValue);
    				}
    				if (searchingName == "ID_ShemaGrShemaGrStruktura")
    				{
    					return String.Format("{0} {1} {2}", searchingName, searchingOperatorString, searchingValue);
    				}
    				if (searchingName == "ID_ShemaGr")
    				{
    					return String.Format("{0} {1} {2}", searchingName, searchingOperatorString, searchingValue);
    				}
    				if (searchingName == "ID_ShemaGrNad")
    				{
    					return String.Format("{0} {1} {2}", searchingName, searchingOperatorString, searchingValue);
    				}
    	...
    	#region Akcije
    	    ...
    	    protected override void OnActionExecuting(ActionExecutingContext filterContext)
            {
                ...
                if (korisnik_data != null)
                {
                   ...
                   _service = new Service.Data.ShemaGrShemaGr(korisnik_data);
                }
    			else
                {
