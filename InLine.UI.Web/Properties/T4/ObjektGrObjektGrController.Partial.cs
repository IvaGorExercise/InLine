...
namespace InLine.UI.Web.Controllers
{
    ...
    public partial class ObjektGrObjektGrController : Controller
    {
    	
    	#region Fields
    
    	 Service.Data.IObjektGrObjektGr _service;
    	...
    	public ActionResult Insert(InLine.Model.Data.Item.ObjektGrObjektGrItem item)
    	{
    		
    		if (item.ID_ObjektGrObjektGr == -1)
    		{
    	...
    	public ActionResult Delete(InLine.Model.Data.Item.ObjektGrObjektGrItem item)
    	{
        ...
        public InLine.Model.Data.ObjektGrObjektGr FindID(long id)
        {
    		InLine.Model.Data.ObjektGrObjektGr rezultat;
            rezultat = _service.FindID(id);
            return rezultat;  
        }
    	
    	
        //public IEnumerable<InLine.Model.Data.ObjektGrObjektGr> GetGridData(string filter, string sort, long? start, long? count, bool customsearch = false)
        //{
        //    IEnumerable<InLine.Model.Data.ObjektGrObjektGr> rezultat;
         //   rezultat = _service.GetGridData(filter, sort, start, count, customsearch);
         //   return rezultat;  
        //}
    
    	public IEnumerable<InLine.Model.Data.ObjektGrObjektGr> GetGridData(string filter, string key, string value, string sort, long? start, long? count, bool customsearch = false)
        {
            IEnumerable<InLine.Model.Data.ObjektGrObjektGr> rezultat;
    	...
    	public ActionResult GetGrid(JqGridRequest request, string key, string value, string viewModel, string field, long? id_master)
        {
    		...
    		public string GetFilter(string searchingName, JqGridSearchOperators searchingOperator, string searchingValue)
    		{
    			...
    			switch (searchingOperator)
    			{
    				...
    				if (searchingName == "ID_ObjektGrObjektGr")
    				{
    					return String.Format("{0} {1} {2}", searchingName, searchingOperatorString, searchingValue);
    				}
    				if (searchingName == "ID_ObjektGr")
    				{
    					return String.Format("{0} {1} {2}", searchingName, searchingOperatorString, searchingValue);
    				}
    				if (searchingName == "ID_ObjektGrNad")
    				{
    					return String.Format("{0} {1} {2}", searchingName, searchingOperatorString, searchingValue);
    				}
    	...
    	#region Akcije
    	    ...
    	    protected override void OnActionExecuting(ActionExecutingContext filterContext)
            {
                ...
                if (korisnik_data != null)
                {
                   ...
                   _service = new Service.Data.ObjektGrObjektGr(korisnik_data);
                }
    			else
                {
