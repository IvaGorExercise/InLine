...
namespace InLine.UI.Web.Controllers
{
    ...
    public partial class OdbitakGrOdbitakGrController : Controller
    {
    	
    	#region Fields
    
    	 Service.Data.IOdbitakGrOdbitakGr _service;
    	...
    	public ActionResult Insert(InLine.Model.Data.Item.OdbitakGrOdbitakGrItem item)
    	{
    		
    		if (item.ID_OdbitakGrOdbitakGr == -1)
    		{
    	...
    	public ActionResult Delete(InLine.Model.Data.Item.OdbitakGrOdbitakGrItem item)
    	{
        ...
        public InLine.Model.Data.OdbitakGrOdbitakGr FindID(long id)
        {
    		InLine.Model.Data.OdbitakGrOdbitakGr rezultat;
            rezultat = _service.FindID(id);
            return rezultat;  
        }
    	
    	
        //public IEnumerable<InLine.Model.Data.OdbitakGrOdbitakGr> GetGridData(string filter, string sort, long? start, long? count, bool customsearch = false)
        //{
        //    IEnumerable<InLine.Model.Data.OdbitakGrOdbitakGr> rezultat;
         //   rezultat = _service.GetGridData(filter, sort, start, count, customsearch);
         //   return rezultat;  
        //}
    
    	public IEnumerable<InLine.Model.Data.OdbitakGrOdbitakGr> GetGridData(string filter, string key, string value, string sort, long? start, long? count, bool customsearch = false)
        {
            IEnumerable<InLine.Model.Data.OdbitakGrOdbitakGr> rezultat;
    	...
    	public ActionResult GetGrid(JqGridRequest request, string key, string value, string viewModel, string field, long? id_master)
        {
            ...
            {
                fieldMain = "ID_OdbitakGrOdbitakGr";
            }
    		...
    		public string GetFilter(string searchingName, JqGridSearchOperators searchingOperator, string searchingValue)
    		{
    			...
    			switch (searchingOperator)
    			{
    				...
    				if (searchingName == "ID_OdbitakGrOdbitakGr")
    				{
    					return String.Format("{0} {1} {2}", searchingName, searchingOperatorString, searchingValue);
    				}
    				if (searchingName == "ID_OdbitakGr")
    				{
    					return String.Format("{0} {1} {2}", searchingName, searchingOperatorString, searchingValue);
    				}
    				if (searchingName == "ID_OdbitakGrNad")
    				{
    					return String.Format("{0} {1} {2}", searchingName, searchingOperatorString, searchingValue);
    				}
    	...
    	#region Akcije
    	    ...
    	    protected override void OnActionExecuting(ActionExecutingContext filterContext)
            {
                ...
                if (korisnik_data != null)
                {
                   ...
                   _service = new Service.Data.OdbitakGrOdbitakGr(korisnik_data);
                }
    			else
                {
